DESCRIPTION
===========
Ogone (http://www.ogone.com) integration module for the Drupal Commerce payment and checkout system.

INSTALLATION INSTRUCTIONS FOR COMMERCE OGONE
============================================

First you should make sure you have an Ogone Merchant account, and it is ready to configure it:

Configuring Goon
------------------
- Log in with your Ogone merchant account (test or prod environment
...
- Enable the module (Go to admin/modules and search for commerce_ogone.
...
- The setting for both of the payment methods should be the same as your credentials at Ogone site. This includes the SHA – IN and OUT phrases. 
- The first option is to select either a test account or production site, we can advise you to implement first of all the test account and perform some tests transactions before moving to production! When moving to production do not forget to change the settings here otherwise an issue will appear. 
...
- The next step is to select the SHA algorithm type; the algorithm type must be the same as the one chosen on ogone site! You can choose between SHA-1, SHA-256, SHA-512. If you didn’t select SHA algorithm on the Ogone backoffice, log in on Ogone and go to Technical Information->Global security parameters and choose the algorithm type.
- The SHA-IN and SHA-OUT should be the same as the one on Ogone site, if you haven’t choose a phrase go to ogone site then Technical Information -> Data and origin verification tab for the SHA-IN phrase and Transaction feedback tab for the SHA-OUT phrase.
- Under Transaction capture method you can choose whether your transactions will be automatically authorized and captured (approved) or you can select Manual to capture transactions later.

- For the Ingenico online gateway you can select the checkbox for preselecting the payment type and choose exactly which of the payment types you would like to implement and to be used by customers. 
...
- For the Ingenico direct gateway" method you have additional option to choose whether or not your customers will be using 3d secure authentication or not. By default all transactions will require 3d secure authentication if you want to by pass the authentication select the radio button for ‘Do not perform 3D-Secure checks and always authorize.’
- If you use the manual capture method and if you want to capture all transactions from the day before, or capture all transactions from today you can set up a rule to do that and you should use the action: ‘Capture from a prior authorization‘ or you can use the example rule in the example module to modify it for your needs, by default this example rule captures all transactions from the day before. This rule is triggered on cron run.
...
Restrictions: 
-------------
It is not recommended to modify the module and you are not allowed to save any customer details, 
only Ogone is PCI compliant and can do so. The client details are directly transmuted to Ogone site, 
this module is not keeping any cardholder information.